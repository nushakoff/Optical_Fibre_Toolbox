function [code] = dispPoynting(F, d)
...
P1 = cross (F.E1, conj(F.H1), 3) + cross (conj(F.E1), F.H1, 3); 
P2 = cross (F.E2, conj(F.H2), 3) + cross (conj(F.E2), F.H2, 3);

EX1 = P1(:,:,1) .* cos(F.FG.PHI1) - P1(:,:,2) .* sin(F.FG.PHI1);
EY1 = P1(:,:,1) .* sin(F.FG.PHI1) + P1(:,:,2) .* cos(F.FG.PHI1);
EZ1 = P1(:,:,3);
EX2 = P2(:,:,1) .* cos(F.FG.PHI2) - P2(:,:,2) .* sin(F.FG.PHI2); 
EY2 = P2(:,:,1) .* sin(F.FG.PHI2) + P2(:,:,2) .* cos(F.FG.PHI2); 
EZ2 = P2(:,:,3);
...
title('P_x');xlabel ('X (\mum)'); ylabel('Y(\mum)'); axis on; view([0 90]);
colorbar;
...
polar(theta,ones(size(theta)) * d/2)
title('P_y');xlabel ('X (\mum)'); ylabel('Y(\mum)'); axis on; view([0 90]);
colorbar;
...
polar(theta,ones(size(theta)) * d/2)
title('P_z');xlabel ('X (\mum)'); ylabel('Y(\mum)'); axis on; view([0 90]);
colorbar;
...
polar(theta,ones(size(theta)) * d/2)
title('P_{total} --- absolute value');xlabel ('X (\mum)'); ylabel('Y(\mum)'); axis on; view([0 90]);
colorbar;
...
title('Power Intensity');xlabel ('X (\mum)'); ylabel('Y(\mum)'); axis on; view([-30 60]);
%colorbar;
